...
public class UserController {	
    ...
    */
    ...
    public String updateInfo(@ModelAttribute("user") User user,HttpSession session) {
		if(userBusiness.updateUser(user)==1){
			return "redirect:/user/haveuserinfo.do";
		}
			System.out.println("用户修改失败");
			return "redirect:/user/updateInfo.do";
    }
    ...
    public String insertInfo(@ModelAttribute("user") User user,@ModelAttribute("userrole") UserRole userrole,HttpSession session) {
    	try {
    ...
    public String insermoretInfo(HttpSession session,HttpServletRequest request) {
		...
		for(int i=0;i<eidlist.size();i++){
			...
			//user.setRid(rid);
			user.setUsername(namelist.get(i).trim()+birthlist.get(i).trim().substring(0, 4));
			userlist.add(user);
		}
		//把每条User新增入数据库
		try {
			for(int j=0;j<userlist.size();j++){
				userBusiness.insertUser(userlist.get(j));
			}
			return "redirect:/user/haveuserinfo.do";
		} catch (Exception e) {
			// TODO: handle exception
     ...
     * @return String
   	 */
		...
		try {
			for(int i=0;i<eidlist.size();i++){
				userBusiness.deleteUserRole(userBusiness.getuidByeid(Integer.parseInt(eidlist.get(i))));
